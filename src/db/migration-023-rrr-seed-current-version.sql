-- migration-023: R&R 업무분장표 데이터를 현재 활성 버전에 반영
-- 사유: migration-022가 "v1.1 존재" 가드에 걸려 시드를 건너뜀.
--       ("새 버전으로 아카이브" 기능으로 v1.1이 이미 만들어져 있었음)
--       현재 활성 버전 항목을 업무분장표(260813) 데이터로 교체한다.
--       기존(구 클론) 항목은 v1.0.0에 그대로 보존됨.

-- 컬럼 보강 (022에서 이미 추가됐지만 안전하게 재확인, 멱등)
...
DECLARE
  ...
  u_gjh INTEGER;  -- 구지향
BEGIN
  SELECT id INTO v_id FROM rrr_versions WHERE is_current = true LIMIT 1;
  IF v_id IS NULL THEN RETURN; END IF;

  -- 이미 시드됐으면(현재 버전에 part 채워진 항목이 있으면) 스킵
  IF EXISTS (SELECT 1 FROM rrr_items WHERE version_id = v_id AND part IS NOT NULL) THEN
    RETURN;
  END IF;
  ...
  IF u_ays IS NULL OR u_nbj IS NULL OR u_gjh IS NULL THEN RETURN; END IF;

  -- 현재 버전 메타 정리 (시행일 2026-08-18)
  UPDATE rrr_versions
     SET effective_date = '2026-08-18',
         title = '2026.08 인력 3명 기준 R&R 재배분',
         change_note = '마케팅본부 업무분장표(260813) 반영. 파트/대분류/주요내용/부담당/주기 표기 추가.'
   WHERE id = v_id;

  -- 현재 버전의 기존 항목 제거 후 재시드 (과거 데이터는 v1.0.0에 보존)
  DELETE FROM rrr_items WHERE version_id = v_id;
